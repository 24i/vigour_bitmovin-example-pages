...
//    ],
  ...
  var actions = {
    'Add subtitle': function() {
      printResult(player.addSubtitle({url: 'http://bitdash-a.akamaihd.net/content/sintel/subtitles/subtitles_en.vtt', id: 'test', lang: 'ru', label: 'Test Subtitle'}));
    },
    'Remove subtitle': function() {
      printResult(player.removeSubtitle('test'));
    },
    'Schedule skippable VAST preroll ad': function() {
      printResult(player.scheduleAd(
  ...
  $.each(actions, function(title, handler) {
    $('#actions').append($('<button type="button" class="btn btn-primary btn-sm">' + title + '</button>').click(handler)).append(' ');
  });
  ...
  for (var member in bitmovin.playerui.UIManager.Factory) {
    if (typeof bitmovin.playerui.UIManager.Factory[member] == 'function') {
